...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = []
                             ...
                             ++ lib.optional (cfg.customComponents != {}) "L+ ${cfg.configDir}/www/custom_components - - - - ${pkgs.linkFarm (lib.mapAttrs (k: v: {
                               name = k; path = v;
                             }) cfg.customComponents)}"
                             ++ lib.optional (cfg.customCards != {}) "L+ ${cfg.configDir}/www/custom_cards - - - - ${pkgs.linkFarm (lib.mapAttrs (k: v: {
                               name = k; path = v;
                             }) cfg.customCards)}";
