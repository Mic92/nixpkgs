      ...
      systemd.services.fwupd-efi = {
        ...
        script = ''
          ...
          for efi in "''${fwupd_efi[@]}"; do
            ${lib.getExe cfg.secureBoot.sbctl} ${lib.optionalString (cfg.secureBoot.configFile != null) "--config ${cfg.secureBoot.configFile}"} sign -o "/run/fwupd-efi/$(basename "$efi").signed" "$efi"
          done
        '';
      };
