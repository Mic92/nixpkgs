...
- [Hydra builder](https://github.com/NixOS/hydra), the build agent of Hydra's Rust queue runner. It replaces the SSH-driven `/etc/nix/machines` setup by connecting to the queue runner over gRPC. Available as [services.hydra-builder](#opt-services.hydra-builder.enable).
...
- `hydra` has been updated to a version where the queue runner and the evaluator
  are rewritten in Rust, tracking what [hydra.nixos.org](https://hydra.nixos.org)
  runs. Several things changed at once:

  - `services.hydra.dbi` is gone. Hydra now reads `HYDRA_DATABASE_URL` instead of
    `HYDRA_DBI`, so use [services.hydra.dbUrl](#opt-services.hydra.dbUrl), which
    takes a libpq URL (`postgres://hydra@localhost:5432/hydra`) rather than a DBI
    string. Both the Perl and the Rust components read it.

  - The queue runner ships in a separate `hydra-queue-runner` package and no
    longer drives build machines over SSH from an `/etc/nix/machines` file:
    build machines instead run the new `hydra-builder` agent, which connects to
    the queue runner's gRPC endpoint and advertises what it can build.
    Deployments relying on
    [services.hydra.buildMachinesFiles](#opt-services.hydra.buildMachinesFiles)
    for remote builds have to enable
    [services.hydra-builder](#opt-services.hydra-builder.enable) on each build
    machine and point it at
    [services.hydra.queueRunner.grpc](#opt-services.hydra.queueRunner.grpc.port),
    which listens on `[::1]:50051` by default.

  - The evaluator ships in a separate `hydra-evaluator` package and is
    configured through
    [services.hydra.evaluatorSettings](#opt-services.hydra.evaluatorSettings).

  - A new `hydra-ws` service streams live build logs to the web interface. It is
    enabled by default and can be turned off with
    [services.hydra.ws.enable](#opt-services.hydra.ws.enable).

  Each Rust service reads its own TOML file under {file}`/etc/hydra` rather than
  taking its settings from {file}`hydra.conf`.
