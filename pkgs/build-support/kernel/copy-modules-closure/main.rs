...
fn main() {
    ...
    let cl = closure( |module| mod_deps(&kernel,allow_missing,&module), &roots );
...
fn copy_firmware(kernel : &KernelData,
    ...
    // of its output.
        let firmware : Vec<OsString> = out
            ...
            .map(|bs| OsString::from_vec(Vec::from(bs)) )
            .collect();
...
fn mod_resolve(kernel : &KernelData,
    ...
    Some(
        names
        .into_iter()
        .map(|bs| OsString::from_vec(bs.to_vec()))
        .collect()
    )
}
...
fn mod_deps(kernel : &KernelData,
    ...
    let dep_lists = modinfo(&kernel,allow_missing,module,"depends")
        .unwrap();
    let raw_deps : Vec<OsString> = dep_lists
        .into_iter()
        .flat_map(|deps|
            deps
            .split(|c| *c == b',')
