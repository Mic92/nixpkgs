# Generates a lookup table mapping bytes (as single-character strings) to their
...
let
  ...
  allBytesFile = ./tests/python/all-bytes-except-null.bin;
  ...
  # Create attribute set mapping each byte character to its hex representation
  # We know the file contains bytes 0x01 through 0xFF in order
  byteAttrs = builtins.genList
    (i: {
